async function setTexts() {
    ...
    let response = await fetch('/ATI/config');
    config = await response.json();
    
    const ci = getCookie('ci');
    let pdata = await fetch('/ATI/pdata',{credentials: 'include'});
    

    profile = await pdata.json();
...
function searchProfile(){
    ...
    console.log("Volver a la pagina principal...")
